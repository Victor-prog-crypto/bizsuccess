(() => {
  ...
  function initializeComponentSelector() {
    if (!document.body) {
      console.error(
        "Bizsaas component selector initialization failed: document.body not found.",
      );
      return;
    }
    ...
    //
    // see: https://github.com/Victor-prog-crypto/bizsuccess/issues/2231
    ...
    function checkForTaggedElements() {
      if (document.body.querySelector("[data-dyad-id]")) {
        ...
        console.debug("Bizsaas component selector initialized");
        return true;
      }
      return false;
    }
    ...
    setTimeout(() => {
      ...
      console.debug(
        "Bizsaas component selector waiting for tagged elements to appear...",
      );
      ...
      timeoutId = setTimeout(() => {
        ...
        if (!document.body.querySelector("[data-dyad-id]")) {
          console.warn(
            "Bizsaas component selector not initialized because no DOM elements were tagged",
          );
        }
      }, INIT_TIMEOUT_MS);
    }, 0);
