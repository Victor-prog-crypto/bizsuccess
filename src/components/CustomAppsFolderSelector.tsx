...
export function CustomAppsFolderSelector() {
  ...
  const handleResetToDefault = async () => {
    try {
      ...
      await fetchCustomAppsFolder();
      showSuccess("Bizsaas apps folder reset successfully");
    } catch (error: any) {
      showError(`Failed to reset Bizsaas Apps folder path: ${error.message}`);
    }
  };
  ...
  const fetchCustomAppsFolder = async () => {
    try {
      ...
      showError(`Failed to fetch Bizsaas apps folder path: ${error.message}`);
    }
  };
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400">
          <p>
            {isPathAvailable
              ? "This is the top-level folder that Bizsaas will store new applications in."
