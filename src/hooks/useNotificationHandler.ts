      ...
      if (currentPermission !== "granted") {
        if (!consentDeniedWarningShownRef.current) {
          ...
          const target = params.sourceLabel
            ...
            : "";
          showWarning(
            `"${params.toolName}"${target} needs your approval. Enable notifications for Bizsaas in your operating system's notification settings.`,
          );
        }
        return;
      }
    ...
    void (async () => {
      ...
      if (window.Notification.permission === "denied") {
        if (!completionDeniedWarningShownRef.current) {
          ...
          showWarning(
            "Enable notifications for Bizsaas in your operating system's notification settings to receive chat completion alerts.",
          );
        }
        return;
      }
