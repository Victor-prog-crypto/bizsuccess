...
const RESTORE_SETTINGS_DOCS_URL =
  "https://www.bizsaas.sh/docs/guides/migrate-restore#restoring-settings-from-backup";
...
function readSettingsForWrite(filePath: string): {
  ...
  try {
    ...
    notifyRendererError({
      message:
        "Bizsaas could not read your existing settings file, so it fell back to default settings.",
      action: {
